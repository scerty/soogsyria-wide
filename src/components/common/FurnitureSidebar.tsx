// src/components/common/FurnitureSidebar.tsx

import React, { useMemo } from 'react';
import { 
  FURNITURE_FILTER_REGISTRY, 
  FurnitureFilterConfig,
} from '../../config/furnitureFilters';
import { FurnitureFilters } from '../../config/furnitureFilters';
...
interface FurnitureSidebarProps {
  ...
  setFilters: (filters: FurnitureFilters | ((prev: FurnitureFilters) => FurnitureFilters)) => void;
  activeTab: string;
  activeSubCategory?: string;
  resetFilters: () => void;
}

export const FurnitureSidebar: React.FC<FurnitureSidebarProps> = ({
  filters,
  setFilters,
  activeTab,
  activeSubCategory,
  ...
  console.log('🎛️ Furniture Sidebar render with filters:', filters);

  const handleFilterChange = (key: string, value: any) => {
    console.log(`🔧 Furniture Filter change: ${key} =`, value);
    ...
    setFilters(prev => {
      ...
      console.log('📝 New furniture filters state:', newFilters);
      return newFilters;
    });
  };
  ...
  const visibleFilters = useMemo(() => {
    const visible = FURNITURE_FILTER_REGISTRY.filter(config => {
      if (!config.visibleWhen) return true;
      return config.visibleWhen(filters, activeTab, activeSubCategory);
    });
    console.log('👁️ Visible furniture filters:', visible.map(f => f.key));
    return visible;
  }, [filters, activeTab, activeSubCategory]);

  const renderFilter = (config: FurnitureFilterConfig) => {
    console.log(`🎨 Rendering furniture filter: ${config.key}`);
    
    const currentValue = (filters as any)[config.key];
    console.log(`📊 Furniture Filter ${config.key} current value:`, currentValue);
      ...
      <div className="border-b pb-4 mb-4">
        ...
        <p className="text-sm text-gray-600 mt-1">
          ...
          {activeSubCategory && (
            <>
              {' - '}
              <span className="text-amber-600">{activeSubCategory}</span>
            </>
          )}
        </p>
      </div>
